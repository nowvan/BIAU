...
function buildMarkdownAST(comments, config) {
  ...
  var linkerStack = new LinkerStack(config).namespaceResolver(comments, namespace => {
  ...
  function generate(depth, comment) {
    ...
    function metaSection(comment) {
      let meta = ['version', 'since', 'copyright', 'author', 'license', 'deprecated'].filter(tag => comment[tag]);
    ...
    if (comment.kind === 'note') {
      return [u('heading', { depth }, [u('text', comment.name || '')])].concat(comment.description);
    }
  ...
  var root = rerouteLinks(linkerStack.link, u('root', generatorComment.concat(config.markdownToc ? tableOfContentsHeading : []).concat(comments.reduce((memo, comment) => memo.concat(generate(2, comment)), []))));
